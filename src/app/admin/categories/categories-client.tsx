  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowForm(!showForm)} className="bg-white text-stone-900 px-5 py-2.5 text-[10px] tracking-[0.35em] uppercase hover:bg-stone-100 transition-colors">
          + New category
        </button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-stone-100 border border-stone-300 p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2 flex gap-3 justify-end">
            <button type="button" onClick={() => setShowForm(false)} className="px-5 py-2.5 border border-stone-700 text-stone-400 text-[10px] tracking-[0.3em] uppercase hover:border-stone-500">Cancel</button>
            <button type="submit" className="px-5 py-2.5 bg-white text-stone-900 text-[10px] tracking-[0.35em] uppercase hover:bg-stone-100">Create</button>
          </div>
        </form>
      )}
          ...
          <tbody className="divide-y divide-stone-300">
            {categories.map((cat) => (
              <tr key={cat.id} className="hover:bg-stone-800/40 transition-colors">
                ...
                <td className="px-4 py-3 text-[10px] text-stone-500 font-mono">{cat.slug}</td>
                <td className="px-4 py-3 text-xs text-stone-400">{cat._count.products}</td>
                <td className="px-4 py-3">
                  <button onClick={() => handleDelete(cat.id)} className="text-[10px] tracking-[0.2em] uppercase text-red-600 hover:text-red-400 transition-colors">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
